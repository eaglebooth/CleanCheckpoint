...
import { connectWallet, readContract, unwrap, writeContract } from "@/lib/genlayer";

type Job = { id: number; client: string; provider: string; title: string; service: string; fee: number; bond: number; status: string; verdict: string; service_deadline: number; challenge_deadline: number; recovery_deadline: number; provider_paid: number; client_refunded: number };
  ...
  return <main>
    <section className="hero" id="home">
      <nav className={`nav shell ${scrolled ? "nav-scrolled" : ""}`}>
        ...
        <div className="nav-actions"><a className="icon-btn" href="https://explorer-studio.genlayer.com/address/0x6e2A785B1699067F8573e765B601f651917D7e47" target="_blank" rel="noreferrer" aria-label="Open CleanCheckpoint contract in GenLayer Explorer" title="Open contract in Explorer"><ExternalLink size={19}/></a><button className="dark-btn" onClick={connect}><Wallet size={17}/>{shortWallet}</button></div>
      </nav>
    ...
    <section className="faq section" id="faq"><div className="shell"><div className="section-kicker center">● FAQ</div><h2 className="center">Transparent by design.</h2>{[["Does CleanCheckpoint judge photos?","No. The MVP deliberately avoids subjective image grading and private home imagery."],["When does GenLayer consensus run?","Only after both parties have submitted role-bound evidence and a dispute is opened."],["What if sources conflict?","The contract enters bounded recovery and returns funds according to the locked rule after the recovery deadline."],["Is transaction finality treated as success?","No. The app re-reads the affected job and shows the authoritative status after every write."]].map(([q,a],i)=><details key={q} open={i===0}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div></section>
    ...
    {workspace && <div className="modal-backdrop"><div className="workspace-panel"><div className="panel-head"><div><div className="section-kicker">Contract workspace</div><h2>Run the real lifecycle</h2></div><button className="icon-btn" onClick={() => setWorkspace(false)}><X/></button></div><div className="workspace-grid">
      ...
      <aside className="state-card"><div className="state-top"><span>Authoritative state</span><button onClick={()=>refresh()}>Refresh</button></div>{job ? <><div className="status-pill">{job.status}</div><h3>#{job.id} · {job.title}</h3><dl><dt>Service</dt><dd>{job.service}</dd><dt>Verdict</dt><dd>{job.verdict}</dd><dt>Fee</dt><dd>{(job.fee/1e18).toFixed(4)} GEN</dd><dt>Bond</dt><dd>{(job.bond/1e18).toFixed(4)} GEN</dd><dt>Provider paid</dt><dd>{(job.provider_paid/1e18).toFixed(4)} GEN</dd><dt>Client refunded</dt><dd>{(job.client_refunded/1e18).toFixed(4)} GEN</dd></dl><p className="mono">Client {job.client}</p><p className="mono">Provider {job.provider}</p></> : <div className="empty-state"><ShieldCheck/><p>Load a job to verify its state directly from the configured contract.</p></div>}</aside>
    </div></div></div>}
