...
import { ArrowRight, Check, ExternalLink, LockKeyhole, ShieldCheck, Wallet, X } from "lucide-react";
import { activeNetwork, connectWallet, explorerUrl, readContract, unwrap, writeContract } from "@/lib/genlayer";
import { lifecycleSteps, walletRole } from "@/lib/lifecycle";

type Job = { id: number; client: string; provider: string; title: string; service: string; fee: string; bond: string; status: string; verdict: string; integrity_status: string; service_deadline: number; challenge_deadline: number; recovery_deadline: number; provider_completion_checkpoint: number; client_response_checkpoint: number; provider_paid: string; provider_refunded: string; client_paid: string; client_refunded: string };
...
export default function AppShell() {
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("workspace") === "open") setWorkspace(true);
  ...
  const role = useMemo(() => walletRole(wallet, job), [wallet, job]);
  const guide = useMemo(() => lifecycleSteps(job, wallet), [job, wallet]);
  const completedSteps = guide.filter(step => step.state === "complete").length;
  ...
  return <main>
    ...
    {workspace && <div className="modal-backdrop"><div className="workspace-panel"><div className="panel-head"><div><div className="section-kicker">Contract workspace · {activeNetwork()}</div><h2>Run the real lifecycle</h2><p className="workspace-note">Follow the role-aware path below. Every accepted write is verified against authoritative contract state before the next step unlocks.</p></div><button className="icon-btn" onClick={() => setWorkspace(false)} aria-label="Close contract workspace"><X/></button></div>
    <section className="lifecycle-guide" aria-label="Job lifecycle progress"><div className="guide-summary"><div><span className={`role-badge role-${role.toLowerCase()}`}>{role === "DISCONNECTED" ? "Wallet not connected" : `${role} wallet`}</span><h3>{job ? `Job #${job.id} progress` : "Start a verified service job"}</h3><p>{job ? `${completedSteps} of ${guide.length} lifecycle stages complete.` : "Connect once, then switch between the stored client and provider wallets when prompted."}</p></div><strong>{Math.round((completedSteps / guide.length) * 100)}%</strong></div><div className="guide-track" style={{"--progress": `${(completedSteps / guide.length) * 100}%`} as React.CSSProperties}/><ol className="guide-steps">{guide.map((step,index)=><li key={step.key} className={`guide-step ${step.state}`}><span>{step.state === "complete" ? <Check size={15}/> : step.state === "locked" ? <LockKeyhole size={14}/> : index + 1}</span><div><b>{step.label}</b><small>{step.detail}</small></div></li>)}</ol></section>
    <div className="workspace-grid">
      ...
      <aside className="state-card"><div className="state-top"><span>Authoritative state</span><button onClick={()=>refresh()}>Refresh</button></div>{job ? <><div className="status-pill">{job.status}</div><h3>#{job.id} · {job.title}</h3><dl><dt>Service</dt><dd>{job.service}</dd><dt>Verdict</dt><dd>{job.verdict}</dd><dt>Evidence integrity</dt><dd>{job.integrity_status || "NOT_EVALUATED"}</dd><dt>Fee</dt><dd>{formatGen(job.fee)}</dd><dt>Bond</dt><dd>{formatGen(job.bond)}</dd><dt>Provider earned</dt><dd>{formatGen(job.provider_paid)}</dd><dt>Provider bond returned</dt><dd>{formatGen(job.provider_refunded)}</dd><dt>Client compensation</dt><dd>{formatGen(job.client_paid)}</dd><dt>Client fee refunded</dt><dd>{formatGen(job.client_refunded)}</dd></dl><p className="mono">Client {job.client}</p><p className="mono">Provider {job.provider}</p></> : <div className="empty-state"><ShieldCheck/><p>Load a job to verify its state directly from the configured contract.</p></div>}</aside>
    </div></div></div>}
